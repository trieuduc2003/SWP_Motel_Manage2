            ...
            <div class="row mb-4">
                <div class="col-md-6 mx-auto text-center">
                    <form action="home" method="get">
                        <input type="text" name="search" placeholder="Search..." class="form-control" style="display:inline-block; width: auto;"
                               value="${not empty searchKeyword ? searchKeyword : ''}"> <!-- Hi?n th? t? khóa tìm ki?m -->

                    <select name="haveDiscount" class="form-control" style="display:inline-block; width: auto;">
                        <option value="all" ${haveDiscount == 'all' ? 'selected' : ''}>All</option> <!-- Gi? tr?ng thái ch?n -->
                        <option value="true" ${haveDiscount == 'true' ? 'selected' : ''}>Have Discount</option> <!-- Gi? tr?ng thái ch?n -->
                    </select>
            ...
            <c:choose>
                <c:when test="${not empty motels}">
                    <c:forEach var="motel" items="${motels}" varStatus="status">
                        <div class="col-md-6 col-lg-3" data-aos="fade-up" data-aos-delay="${status.index * 100}">
                            <a href="room?action=list-user&motelId=${motel.motelId}" class="unit-9">
                                <div class="image" style="background-image: url('images/img_1.jpg');"></div>
                                <div class="unit-9-content">
                                    <h2>${motel.motelName}</h2>
                                    <p>Click to view rooms</p> <!-- Optional additional text -->
                                </div>
                            </a>
                        </div>
                ...
                <c:otherwise>
                    <div class="col-md-12 text-center mt-5" data-aos="fade-up">
                        <h2>No Motels Found</h2>
                    </div>
                </c:otherwise>
            </c:choose>

            <div class="col-md-12 text-center mt-5" data-aos="fade-up">
                <a href="home" class="btn btn-primary">Browse All Apartments</a>
            </div>
        </div>
...
<jsp:include page="footer.jsp"></jsp:include>a